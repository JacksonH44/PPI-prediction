...
class FrustrationPlotter(Plotter):
    ...
    def _feature_init(self):
        ...
        self._ylabel = "$\Delta$ Frustration Index"  # noqa: W605

        # Create data object
        stats_df = pd.read_csv(
            self._stats_file_path, usecols=["symbol", "avg_fi_i_1", "avg_fi_i_2"]
        )
        neg_fi_i_1 = []
        neg_fi_i_2 = []
        pos_fi_i_1 = []
        ...
        data = {
            ...
            "Class": ["Cancer Driver (+)"] * len(pos_fi_i_1)
            + ["Cancer Driver (-)"] * len(neg_fi_i_1)
            + ["Partner (+)"] * len(pos_fi_i_2)
            + ["Partner (-)"] * len(neg_fi_i_2),
        }
