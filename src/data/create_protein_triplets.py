...
def find_triplets(infile, positive):
    ...
    if positive:
        reference_isoforms = ppi_df[
            (ppi_df["Category"] == "reference")
            & (ppi_df["Interaction_Found"] == "positive")
        ]
    else:
    ...
    for gene_symbol in reference_isoforms["Gene_Symbol"].unique():
        ...
        alternative_isoforms = ppi_df[
            (ppi_df["Gene_Symbol"] == gene_symbol)
            & (ppi_df["Category"] == "alternative")
        ...
        for _, alt_row in alternative_isoforms.iterrows():
            ref_isoform = reference_isoforms[
                (reference_isoforms["Gene_Symbol"] == gene_symbol)
                & (reference_isoforms["Interactor_ID"] == alt_row["Interactor_ID"])
            ]
            if ref_isoform.empty:
                continue
            logging.debug(
                f"Alternative isoform:\n{alt_row}\nReference isoform:\n{ref_isoform}"
            )
            ...
            perturbation = (
                ref_isoform["Interaction_Found"].values[0]
                != alt_row["Interaction_Found"]
            )
            logging.debug(
                f"Adding observation ({ref_id}, {alt_id}, {bait_id}, {perturbation})"
            )
            observations.append([ref_id, alt_id, bait_id, perturbation])
    observation_df = pd.DataFrame(
        observations, columns=["ref_ID", "alt_ID", "bait_ID", "perturbation"]
    )
...
def parse_command_line():  # pragma: no cover
    ...
    parser.add_argument(
        "outfile", type=str, help="Absolute path of the CSV file to write to"
    )
    parser.add_argument(
        "-p",
        "--positive",
...
def main():  # pragma: no cover
    ...
    logfile = (
        ...
        else os.path.join(os.getcwd(), "logs/create_protein_triplets.log")
    )
