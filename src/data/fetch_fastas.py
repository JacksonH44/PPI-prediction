...
def fetch_fastas(input_file, output_folder):
    ...
    id_df = pd.read_excel(
        input_file,
        sheet_name="2A-Isoforms tested in Y2H",
        usecols=["Isoform_ID", "GenBank_Accession"],
    )
    ...
    for accession in accession_numbers:
        ...
        logging.debug(
            f"Request for transcript from {accession} had API response {response.status_code}"
        )
        if response.status_code == 200:
            fasta_sequence = response.text
            isoform_id = (
                id_df[id_df["GenBank_Accession"] == accession]["Isoform_ID"].values
            )[0]
...
def parse_command_line():  # pragma: no cover
    ...
    parser.add_argument(
        "outfolder", type=str, help="Output folder path (requires absolute path)"
    )
    parser.add_argument(
        "-v",
        "--verbose",
